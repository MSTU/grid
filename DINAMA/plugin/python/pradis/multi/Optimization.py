...
#		self.constraitmethod = pl[6]
		...
		for i in self.vl:
			...
			self.vl0.append (float(i.Value0))
			self.bounds.append ((float(i.Min), float(i.Max)))
...
#			print 'xa=',xa,' xb=',xb
			...
			self.xopt = self.aNormX(self.xopt)
...
#			print 'vl0=',vl0
		...
		elif self.method == 'Succesive':
			self.xopt = self.successive (self.Objective, vl0, self.xtol, self.ftol, self.maxfun)
...
#		print 'point2'
	...
	def successive (self, objective, vl0, _xtol, _ftol, _maxfun):

		bounds = self.NormBounds (self.bounds)
		for i in xrange (200):
			self.xopt = fmin (objective, vl0, xtol=_xtol, ftol=_ftol, maxfun=_maxfun)
#			self.xopt = fmin_powell (objective, vl0, xtol=_xtol, ftol=_ftol, maxfun=_maxfun)
#			(self.xopt, f, d) = fmin_tnc (objective, vl0, approx_grad=1, bounds = bounds, ftol=_ftol, xtol=_xtol, epsilon=self.epsilon, maxfun=_maxfun)
#			(self.xopt, f, d) = fmin_l_bfgs_b (self.Objective, vl0, approx_grad=1, bounds = bounds, epsilon=self.epsilon, maxfun=self.maxfun)
...
#		print 'ma.run'
		if self.fl!=[]:
			...
			c = 0   # для случая без внешнего решателя
			
		if c != 0:
			print 'Optimization: Error code = ', c

#			return 1.0  
#Optimization.MaxValue/10000.0
			value = 2.0;
		else:
#			print 'ma.F'
			value = self.F (x, self.ma)		
#			print 'end ma.F value  = ', value
...
#		print 'py writeHistory'
		
		if self.isCobyla!=1:
			self.ma.writeHistory(self.iteration)

#		print 'end writeHistory'
