...
class REACTION(model):
	...
	#		print 'A=', V[5]
			...
			lnq_v=kv+Ma*log(nA)+Mb*log(nB)+log (k*Tin)
			lnq_r=kr+Mc*log(nC)+Md*log(nD)+log (k*Tin)
			
			print 'lnq_v = ', lnq_v, '  lnq_r = ', lnq_r
			q = -exp (lnq_v)+exp (lnq_r)
...
#		Kder = (10.**20.)*  (k2-k1)/(t2-t1)
#		Kvel = Kder * (Kvel-20.)+10.**20.0
#	else:
	Kvel2 = exp( Kvel)
	Kder = Kvel2*  (k2-k1)/(t2-t1)
