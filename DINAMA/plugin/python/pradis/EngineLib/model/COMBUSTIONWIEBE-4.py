...
#HELP			<description>
#HELP				<russian>Соотношение воздух/топливо</russian>
...
#    conrod  = PAR[3]	# conrod length
		...
		T0 = fp.TG(PAR,11)+273.15
		MM = fp.MM(PAR,11)  #28.98/1000
		Cp = fp.Cp(PAR,11)         #1005
		R = fp.R(PAR,11)          #8.31441; 
		# print 
		# print 'test: -0.3'
...
#  //ChamberVolume
		...
		derPhi_V= Ap * stroke*sin (crank_phi)/2.0
		# print 
		# print 'test: 1.1'
...
#  // Convection parameters
		...
		if T+T0>eps:
			m = (P+Pa)*V1*MM / (R*(T+T0))
			der_m = der_P*V1*MM/(R*(T+T0)) + der_V*(P+Pa)*MM / (R*(T+T0))+der_T*(-(P+Pa)*V1*MM / (R*((T+T0)**2)))
			derP_m = V1*MM / (R*(T+T0))
			derPhi_m = (P+Pa)*MM / (R*(T+T0))*derPhi_V
			derPhi_T = (-(P+Pa)*V1*MM / (R*((T+T0)**2)))
		else:
			T1 = eps
#			derT_m = (-(P+Pa)*V1*MM / (R*((eps)**2)))
#			m = m0 + derT_m*(T1-(T+T0))
			m = (P+Pa)*V1*MM / (R*(eps))
			der_m = der_P*V1*MM/(R*(T1)) + der_V*(P+Pa)*MM / (R*(T1))
			derP_m = V1*MM / (R*(T1))
			derPhi_m = (P+Pa)*MM / (R*(T1))*derPhi_V
			derPhi_T = 0.0
...
#  //Combustion
		...
		if ((abs(burning-1.0)<eps)):
			...
			m_fuel_burnt= m_fuel_start *(1 - expf);
...
#		Y[2+Next*1+dyv]  = 1.0    				# test
		...
		Y[2+Next*1+dyv]  = (Cp-R) * der_m   	 #  T		
		Y[2+Next*1+dya]  = (Cp-R) * m    		#  T'
